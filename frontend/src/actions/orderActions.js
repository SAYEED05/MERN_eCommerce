...
//CASH RECEIVED ACTION
export const cashReceived = (order) => async (dispatch, getState) => {
  try {
    dispatch({
      type: ORDER_CASH_RECEIVED_REQUEST,
    });
    ...
    const { data } = await axios.put(
      `/api/orders/${order._id}/cashreceived`,
      {},
      config
    );

    dispatch({
      type: ORDER_CASH_RECEIVED_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: ORDER_CASH_RECEIVED_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
